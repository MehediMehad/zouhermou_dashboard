...
import { useGetAllPromoterQuery } from "@/Redux/Api/userApi";
...
const Promoter = () => {
  ...
  const { data: getResponse, isLoading } = useGetAllPromoterQuery(queryParams); //
  if (isLoading) return <Loading />;
  console.log("😍😍",getResponse);
  
  const promoters = getResponse?.data.data || [];
  const totalPages = getResponse?.data.meta?.total_pages ?? 0;

  const openPagination = promoters.length > 0 && totalPages > 1;

  return (
    <div>
      <PromoterTable promoters={promoters} openPagination={openPagination} />
