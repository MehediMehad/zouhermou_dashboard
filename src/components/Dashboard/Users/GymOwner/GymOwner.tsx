...
import { useGetAllGymOwnerQuery } from "@/Redux/Api/userApi";
import GymOwnerTable from "./GymOwnerTable";
...
const GymOwner = () => {
  ...
  const { data: getResponse, isLoading } = useGetAllGymOwnerQuery(queryParams); //
  if (isLoading) return <Loading />;
  console.log(getResponse?.data.data);
  
  const gymowners = getResponse?.data.data || [];
  const totalPages = getResponse?.data.meta?.total_pages ?? 0;

  const openPagination = gymowners.length > 0 && totalPages > 1;

  return (
    <div>
      <GymOwnerTable gymowners={gymowners} openPagination={openPagination} />
